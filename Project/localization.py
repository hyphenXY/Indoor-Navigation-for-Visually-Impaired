...
def GroundTruthData(directory_gt, floor, test_location, num_files):
    ...
    dataframe_gt = pd.concat(
        [df.set_index("location") for df in dataframes_gt], axis=1, join="outer"
    ).reset_index()
...
def GPPredictedData(directory_pd, num_files):
    ...
    # Merge all dataframes on 'location'
    dataframes_pd = pd.concat(
        [df.set_index("location") for df in dataframes_pd], axis=1, join="outer"
    ).reset_index()
...
def knnLocalizationPredictedData(y1_test, y2_test, X_test, y1_pred, y2_pred, path_3):
    ...
    r2 = pd.concat(
        [pd.DataFrame({"SP_cordX": y1_pred}), pd.DataFrame({"SP_cordY": y2_pred})],
        axis=1,
    )
    ...
    result["dist_localization_error_pd"] = np.round_(
        np.sqrt(
            (result["cordX"] - result["SP_cordX"]) ** 2
            + (result["cordY"] - result["SP_cordY"]) ** 2
        ),
        2,
    )
    result["loc"] = (result["cordX"].map(str)).str.cat(
        result["cordY"].map(str), sep=","
    )
    result["Pred_loc"] = (result["SP_cordX"].map(str)).str.cat(
        result["SP_cordY"].map(str), sep=","
    )
    result = result[["loc", "Pred_loc", "dist_localization_error_pd"]]
    # print("Part 1: Result on GP predicted test dataset ==========")

    print(
        "Localziation_mse_knn_pd_predicted",
        np.mean(result["dist_localization_error_pd"]) * 0.305,
        "m",
    )
...
def knnLocalizationGroundTruthData(y1_gt, y2_gt, X_gt, y1_pred_gt, y2_pred_gt, path_4):
    ...
    r2 = pd.concat(
        [
            pd.DataFrame({"GPP_cordX": y1_pred_gt}),
            pd.DataFrame({"GPP_cordY": y2_pred_gt}),
        ],
        axis=1,
    )
    result = pd.concat([r1, r2.set_index(r1.index)], axis=1)
    ...
    result["dist_localization_error_gt"] = np.round_(
        np.sqrt(
            (result["cordX"] - result["GPP_cordX"]) ** 2
            + (result["cordY"] - result["GPP_cordY"]) ** 2
        ),
        2,
    )
    result["loc"] = (result["cordX"].map(str)).str.cat(
    ...
    result["Pred_loc"] = (result["GPP_cordX"].map(str)).str.cat(
        result["GPP_cordY"].map(str), sep=","
    )
    result = result[["loc", "Pred_loc", "dist_localization_error_gt"]]
    # print("Part 2: Result on Ground truth test dataset ==========")

    print(
        "Localziation_mse_knn_gt_predicted",
        np.mean(result["dist_localization_error_gt"]) * 0.305,
        "m",
    )
...
X_train, X_test, y1_train, y1_test = train_test_split(
    X_pd, y1_pd, test_size=0.2, random_state=42
)

# print("X_train", X_train)
# print("xpd", X_pd)

# print("sdgfgwfgfgagfigffggffiugfrshape", X_train.shape)

mae_pd_x, mae_gt_x, y1_pred, y1_pred_gt = knnXaxis(
    X_train, y1_train, X_test, y1_test, y1_gt
)

# print(X_pd)
# #### Split the data set for training and testing ###########
X_train, X_test, y2_train, y2_test = train_test_split(
    X_pd, y2_pd, test_size=0.2, random_state=42
)


mae_pd_y, mae_gt_y, y2_pred, y2_pred_gt = knnYaxis(
    X_train, y2_train, X_test, y2_test, y2_gt
)
...
mse_knn_pd = knnLocalizationPredictedData(
    y1_test, y2_test, X_test, y1_pred, y2_pred, path_3
)
mse_knn_gt = knnLocalizationGroundTruthData(
